...
const SORT_OPTIONS: { label: string; key: keyof DistrictRow }[] = [
...
export default function DistrictListPage() {
  ...
  const fetchRows = React.useCallback(
    async (start = 0, isReset = false) => {
      ...
      try {
        ...
        const mapped: DistrictRow[] = raw.map((r: any) => ({
          name: r.name,
          district: r.district,
          modified: r.modified,
        }));
        ...
        if (isReset) setError(err.response?.status === 403 ? "Unauthorized" : "Failed to fetch districts.");
      } finally {
        setLoading(false);
  ...
  const requestSort = (key: keyof DistrictRow) => {
    setSortConfig((prev) => ({
      key,
  ...
  const renderListView = () => (
    <div className="stock-table-container thin-scroll-x" style={{ overflowX: "auto" }}>
      <table className="stock-table" style={{ minWidth: "600px", whiteSpace: "nowrap" }}>
        <thead>
          <tr>
  ...
  const renderGridView = () => (
    <div className="equipment-grid grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {rows.length ? (
        ...
        <p className="col-span-full py-8 text-muted-foreground">No districts found.</p>
      )}
    </div>
  );
