...
const SORT_OPTIONS: { label: string; key: keyof VillageRow }[] = [
...
export default function VillageListPage() {
  ...
  const fetchRows = React.useCallback(
    async (start = 0, isReset = false) => {
      ...
      try {
        ...
        const mapped: VillageRow[] = raw.map((r: any) => ({
          name: r.name,
          village: r.village,
          taluka: r.taluka,
          district: r.district,
        ...
        if (isReset) setError(err.response?.status === 403 ? "Unauthorized" : "Failed to fetch villages.");
      } finally {
        setLoading(false);
  ...
  const requestSort = (key: keyof VillageRow) => {
    setSortConfig((prev) => ({
      key,
      ...
      <table className="stock-table" style={{ minWidth: "1000px", whiteSpace: "nowrap" }}>
        <thead>
          <tr>
            ...
            <th style={{ cursor: "pointer", minWidth: 200 }} onClick={() => requestSort("village")}>
              Village
            </th>
            ...
            rows.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  <td style={{ minWidth: 200 }}>{row.village}</td>
  ...
  const renderGridView = () => (
    <div className="equipment-grid grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {rows.length ? (
        ...
        <p className="col-span-full py-8 text-muted-foreground text-center">No villages found.</p>
      )}
    </div>
  );
